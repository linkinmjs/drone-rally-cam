# Modified from GodotDrone (GPL-3.0, (c) Cykyrios), 2026: now a Resource so profiles can be
# saved as .tres files and assigned to a drone in the inspector.
...
@export var pitch_rate := 300.0
@export var roll_rate := 300.0
@export var yaw_rate := 250.0
@export var pitch_rc := 60.0
@export var roll_rc := 60.0
@export var yaw_rc := 60.0
@export var pitch_expo := 0.2
@export var roll_expo := 0.2
@export var yaw_expo := 0.2

@export var rate_curve := RateCurve.ACTUAL
