...
## Projects a world-space direction through a rectilinear camera to viewport coordinates.
## Returns Vector2(NAN, NAN) when the direction points behind the camera.
static func project_direction(camera: Camera3D, direction: Vector3) -> Vector2:
	var point := camera.global_position + direction
