# Modified from GodotDrone (GPL-3.0, (c) Cykyrios) via drone-simulator, 2026: works with any rectilinear
# Camera3D.
...
func _draw_camera_horizon() -> void:
	...
	if show_ladder:
		for elevation in LADDER_STEPS:
			...
			var p_mid := _to_local(HUDDraw.project_direction(camera, mid))
			var p_left := _to_local(HUDDraw.project_direction(camera, left))
			var p_right := _to_local(HUDDraw.project_direction(camera, right))
